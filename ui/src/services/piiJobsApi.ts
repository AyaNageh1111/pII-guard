...
export const fetchPiiJobs = async (filters?: {
  ...
  try {
    const apiUrl = import.meta.env.VITE_PII_DETECTOR_API_ENDPOINT.replace(
      /'/g,
      ""
    );
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch PII jobs: ${response.status} ${response.statusText}`
      );
    }
...
export const mockPiiJobs: Job[] = [
  ...
  },
  {
    ...
    error_details:
      "Failed parsing Nginx access log line 1 due to missing content length",
  },
  {
    id: "job_12347",
    version: "1.0.0",
